...
public class DettaglioErrore implements IDettaglio {
	private String parteVariabile,tipoRecord;
	private int numeroProgressivo;
	public DettaglioErrore(String tipoRecord, int numeroProgressivo, String parteVariabile){
	...
	public List<String> getParteVariabile() {
		...
		String[] pvs = parteVariabile.split("\\|");
		for(int i = 0; i<pvs.length-1; i += 2){
			pv.add(pvs[i] + "|" + pvs[i+1]);
		}
		return pv;
	}
